...
const MOOD_PALETTES = {
  ...
  pastel: () => generateFromHue(Math.random() * 360, 45, 80),
  dark: () => generateFromHue(Math.random() * 360, 50, 25),
  earthy: () => generateFromHue(Math.random() * 40 + 20, 40, 40),
  ocean: () => generateFromHue(Math.random() * 40 + 180, 65, 50),
  sunset: () => generateFromHue(Math.random() * 30 + 10, 80, 55),
  forest: () => generateFromHue(Math.random() * 40 + 100, 55, 40),
  candy: () => generateFromHue(Math.random() * 60 + 300, 70, 70),
  midnight: () => generateFromHue(Math.random() * 40 + 220, 60, 20),
  autumn: () => generateFromHue(Math.random() * 30 + 15, 65, 45),
  spring: () => generateFromHue(Math.random() * 60 + 80, 60, 65),
  winter: () => generateFromHue(Math.random() * 30 + 200, 30, 75),
  neon: () => generateFromHue(Math.random() * 360, 100, 55),
  retro: () => generateFromHue(Math.random() * 40 + 30, 55, 50),
  luxury: () => generateFromHue(Math.random() * 20 + 35, 45, 30),
...
function generateFromHue(baseHue, baseSat, baseLit) {
  const colors = [];
...
export function generatePaletteFromMood(mood) {
...
export function exportAsCSS(colors, name = 'palette') {
  const vars = colors.map((c, i) => `  --${name}-${i + 1}: ${c.hex};`).join('\n');
  return `:root {\n${vars}\n}`;
}

export function exportAsJSON(colors, name = 'palette') {
...
export function exportAsSVG(colors) {
  const width = 500;
